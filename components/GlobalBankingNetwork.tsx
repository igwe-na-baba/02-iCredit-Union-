...
import { getBankIcon, SearchIcon } from './Icons';
...
const BankCard: React.FC<{ name: string }> = ({ name }) => {
    const BankLogo = getBankIcon(name);
    return (
        <div className="bg-slate-200 p-4 rounded-xl shadow-digital flex flex-col items-center justify-center text-center h-32">
...
export const GlobalBankingNetwork: React.FC = () => {
                ...
                <div className="p-6">
                    {filteredBanks.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {filteredBanks.map(bank => <BankCard key={bank.name} name={bank.name} />)}
                        </div>
                    ) : (
