...
import { CheckCircleIcon, ClockIcon, SendIcon, ArrowsRightLeftIcon, ShieldCheckIcon, ScaleIcon, GlobeAltIcon, BankIcon } from './Icons';
                ...
                >
                  {isStepCompleted ? (
                    ...
                    React.cloneElement(step.icon, {
                        className: `w-6 h-6 ${isCurrentStep && isFlaggedStep ? 'text-white' : isCurrentStep ? 'text-white' : ''}`
                    })
                  )}
                </div>
                <p
                  className={`mt-2 text-xs text-center font-medium w-24 ${
                    isStepCompleted || isCurrentStep ? 'text-slate-700' : 'text-slate-400'
                  }`}
                >
                  {step.label}
                </p>
              ...
              {index < steps.length - 1 && (
                <div
                  className={`flex-1 h-1 mx-2 mt-5 transition-colors duration-300 ${
                    isStepCompleted ? 'bg-green-500' : 'bg-slate-200'
                  }`}
                ></div>
              )}
            </React.Fragment>
